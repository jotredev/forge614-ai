...
| T10 | PR, CI, publicación, instalación y activación en la Mac | bajo (T10a) / alto, datos reales (T10b) | T10a: Claude Code · Sonnet 5 · low; T10b: Claude Code · Opus 5.5 · high | T10a low; T10b datos reales con control de procesos |
...
- [ ] **Step 4 (docs y versión, prompt aparte, sesión nueva, dos commits)**

**Código aprobado:** `420275a`, idéntico al parche (diff sin las líneas `index`); suite medida en copia 672 pass / 10 skip / 0 fail, typecheck 0. Agente T8 código: 0,42 M tokens ≈ $0,27, 4,2 min, 0 rondas.

**Método (como T7 docs):** datos verificados contra `420275a` y lugar exacto de cada cambio; el agente redacta, el orquestador verifica cada frase. Agente: Claude Code · Sonnet 5 · medium, sesión nueva, etiqueta `[Engram · T8 · docs]`.

**Datos (verificados contra `420275a`):**
- Base nueva: `MemoryWorkspace.init()` deja en el nivel 11 (memoria inteligente, que incluye ecosistema, sesiones y refuerzo) una base cuyo archivo no existía; no crea respaldo porque no hay nada que copiar. Todo camino que crea el espacio pasa por ahí (`init`, `init --json`, `init` de terminal, `project-create`…). Una base existente, aun sin su archivo de configuración, nunca se migra en `init`: se activa con `intelligence-enable`, con respaldo y verificación. El servidor MCP no crea ni migra bases. `init --json` en una base nueva devuelve `reinforcementEnabled: true`, y un `intelligence-enable` posterior responde `{ "enabled": true, "schema": 11, "migrated": false, "backup": null }`.
- `init` de terminal: con una base nueva ya no pregunta por el refuerzo y escribe «La base nueva se creará con la memoria inteligente (esquema 11), que ya incluye sesiones y el refuerzo de recuerdos.»; con una base existente sin refuerzo, la pregunta sigue igual.
- `sessionProjectId` (`memory_save`): descripción propia en `tools/list`: «Only with scope shared and a sessionId: the projectId that memory_session_start returned.»; ya no hereda la del id de recuerdo. Su validación no cambia.
- La replicación de grupos (formato 4) pasa a la versión 1.8.0.

**Dónde va cada cambio (es y en con el mismo contenido):**

*Commit 1 (documentación):*
1. `CHANGELOG.md` (sección `## 1.7.0 — en desarrollo`): en la viñeta **Esquema 11**, la frase «Se activa solo con `intelligence-enable`, …» pasa a decir que en una base existente se activa solo con `intelligence-enable` (el resto de la frase igual) y que una base nueva nace en el nivel 11; en la viñeta **Bloque de arranque**, la frase final «Sin cambios en MCP ni en el protocolo (las versiones 1 a 3 siguen anunciando el formato 1).» pasa a «Sin cambios en MCP; el protocolo v4 anuncia el formato 2 y las versiones 2 y 3 siguen anunciando el formato 1 (la 1 no anuncia ninguno).»; en la viñeta **Protocolo v4**, «tiene dos salidas» pasa a tres salidas (completo, MCP y descripciones de campos) sin repetir después la frase de las descripciones; y una viñeta nueva «**Bases nuevas con memoria inteligente:**» justo antes de `- La replicación de grupos (formato 4) pasa a la versión 1.8.0.`, con los datos de base nueva, `init` de terminal y `sessionProjectId`.
2. `docs/es/01-instalacion-y-primeros-pasos.md` y `docs/en/01-installation-and-getting-started.md`: la frase «Solo prepara memoria, sincronización PostgreSQL opcional y refuerzo opcional.» / su equivalente en inglés: una base nueva nace con la memoria inteligente (incluye sesiones y refuerzo); en una base existente sin refuerzo, el `init` de terminal sigue preguntando, y la memoria inteligente se activa con `intelligence-enable` (capítulo 05).
3. `docs/es/03-referencia-cli.md` y `docs/en/03-cli-reference.md`: en el bloque de comandos, la descripción de `intelligence-enable` («habilita explícitamente la memoria inteligente (esquema 11); respalda antes de migrar» / "explicitly enable memory intelligence (schema 11); backs up before migrating") agrega que una base nueva ya nace así; misma alineación de columnas.
4. `docs/es/04-sdk-typescript.md` y `docs/en/04-typescript-sdk.md`: en el párrafo de `memoryProtocol()`, «versión `1`» pasa a versión `1` por defecto, y `memoryProtocol(2)`, `(3)` y `(4)` devuelven esas versiones (capítulo 09).
5. `docs/es/05-arquitectura-interna-y-formulas.md` y `docs/en/05-internal-architecture-and-formulas.md`: la última frase del párrafo del esquema 11 («El nivel 11 se activa solo de forma explícita con `intelligence-enable`, con respaldo y verificación; el servidor MCP nunca migra la base.») pasa a: una base nueva (cuyo archivo no existía) nace en el nivel 11 sin respaldo; en una base existente, aun sin configuración, se activa solo con `intelligence-enable`, con respaldo y verificación; el servidor MCP nunca migra la base.
6. Réplica a 1.8.0 (solo cambia el número; el resto de cada frase igual): `docs/es/06-resolucion-de-errores.md` y `docs/en/06-troubleshooting.md` (línea de `SYNC_ECOSYSTEM_UNSUPPORTED`), `docs/es/07-glosario.md` y `docs/en/07-glossary.md` (entrada de la réplica PostgreSQL), `docs/es/11-ambitos-y-ecosistemas.md` y `docs/en/11-scopes-and-ecosystems.md` (punto «Réplica PostgreSQL» / "PostgreSQL replica"), y el párrafo «Límites de esta entrega» de la sección 1.6.0 del capítulo 08. La sección `## 1.6.0` del CHANGELOG **no** se toca (es historia).
7. `docs/es/08-limites-y-roadmap.md` y `docs/en/08-boundaries-and-roadmap.md`: una sección nueva `## Capacidades incorporadas en v1.7.0` / `## Capabilities added in v1.7.0` justo después del párrafo «Límites de esta entrega» de la de 1.6.0 y antes de `## No objetivos explícitos` / su equivalente en inglés (búscalo): viñetas cortas con esquema 11 y su activación (bases nuevas incluidas), filtro de secretos, búsqueda híbrida y parecidos, sesiones interrumpidas, reglas del tablero (nota de estado, proyecto fuente, bajar), bloque de arranque (formato 2) y protocolo v4 (instrucciones MCP y descripciones de campos), remitiendo a los capítulos 05, 09, 10 y 11; y como límite, la réplica de grupos en 1.8.0. En «Protocolo público de memoria: estado de integración», después de la primera frase, una frase: la versión 4 (desde 1.7.0) es el manual de la memoria inteligente y el valor por defecto sigue en 1 hasta que Engines la acepte. No agregues nombres de productos.
8. `docs/es/09-protocolo-publico-de-memoria.md` y `docs/en/09-public-memory-protocol.md`: en la sección de la versión 4, «el manual es la única fuente y de él salen tres salidas.» pasa a nombrar las tres (`instructions`, `mcpInstructions` y las descripciones de campos de `tools/list`), y la viñeta de `startupContext` sale de la lista y queda como frase aparte («Además, `startupContext` anuncia…»); en el párrafo **Descripciones de campos**, `sessionProjectId` entra en la lista de `memory_save` con lo que significa.
9. `docs/es/10-contexto-de-inicio.md` y `docs/en/10-startup-context.md`: quitar una de las dos líneas en blanco antes de `## Errores seguros` / `## Safe errors`; y en el ejemplo JSON del formato 2, cambiar `a1b2c3d4` por `3f2b9c1e-7d4a-4e8b-9a61-2c5d8e0f4b17`, `e5f6a7b8` por `b8e4d2a0-5c19-4f37-8e2d-6a9b1c3e7f50`, `263/5000` por `319/5000`, `"chars": 263` por `"chars": 319` y `"essentials": 69` / `"index": 88` por `"essentials": 97` / `"index": 116` (calculado por el orquestador: 102 + 2 + 97 + 2 + 116 = 319).
10. `docs/notion-map.json`: `"notionSyncPending": true` en las dos entradas del capítulo 01 (existen y no lo tienen); las demás tocadas ya lo tienen y 04 y 05 no tienen entrada (nunca se crean).

Verificación del commit 1: `git diff --check` sin salida y `git diff --stat` solo con esos 22 archivos (CHANGELOG, 01–11 salvo 02 en es y en, y `notion-map.json`).

```bash
git add CHANGELOG.md docs/notion-map.json docs/es/01-instalacion-y-primeros-pasos.md docs/en/01-installation-and-getting-started.md docs/es/03-referencia-cli.md docs/en/03-cli-reference.md docs/es/04-sdk-typescript.md docs/en/04-typescript-sdk.md docs/es/05-arquitectura-interna-y-formulas.md docs/en/05-internal-architecture-and-formulas.md docs/es/06-resolucion-de-errores.md docs/en/06-troubleshooting.md docs/es/07-glosario.md docs/en/07-glossary.md docs/es/08-limites-y-roadmap.md docs/en/08-boundaries-and-roadmap.md docs/es/09-protocolo-publico-de-memoria.md docs/en/09-public-memory-protocol.md docs/es/10-contexto-de-inicio.md docs/en/10-startup-context.md docs/es/11-ambitos-y-ecosistemas.md docs/en/11-scopes-and-ecosystems.md
git commit -m "docs: new databases start with memory intelligence, group replication moves to 1.8.0, 1.7.0 capabilities and consistency fixes"
```

*Commit 2 (versión):* `package.json` `"version": "1.6.0"` → `"1.7.0"` (probado: la suite no cambia); en `CHANGELOG.md`, `## 1.7.0 — en desarrollo` → `## 1.7.0` y la línea «Memoria inteligente; esta sección crece tarea por tarea.» → un resumen de una línea al estilo de la de 1.6.0 (por ejemplo: memoria inteligente: esquema 11, búsqueda híbrida, parecidos y secretos al guardar, sesiones interrumpidas, reglas del tablero, bloque de arranque y protocolo v4). Después: `bun test` (672 pass / 10 skip / 0 fail) y `bun run typecheck`.

```bash
git add package.json CHANGELOG.md
git commit -m "chore: version 1.7.0 and changelog"
```

**Resultado (2026-09-25):** `52289ec` (22 archivos) y `f181a1d` (versión 1.7.0), revisados frase por frase contra `420275a`; suite en copia sobre `f181a1d` 672 pass / 10 skip / 0 fail, typecheck 0. Agente: 1,36 M tokens ≈ $0,76, 3,1 min, 0 rondas. **T8 cerrada.** Impacto en el checklist de agentes: **Sí**, D-T8-8 (fila del refuerzo en el contrato del ecosistema, reglamento 1.1.0); el resto no cambia cómo se integra un asistente.

Reglas de redacción: cada frase debe poder comprobarse en el código de `420275a`; si un dato de esta lista no coincide con el código, no lo escribas y repórtalo. Sin nombres de productos externos (acta 0012). Decimales con coma y miles con espacio en español. No cambies ningún otro archivo ni otras frases.
...
**Resultado T10a ronda 1 (2026-09-25):** PR #11 abierto; `Verify` falló en ubuntu y macOS (pull_request y push) con 2 pruebas de PostgreSQL; el agente se detuvo sin fusionar ni crear el tag (parada correcta). CodeRabbit omitió la revisión (107 archivos > 100).

**Causa (verificada por el orquestador en copias con PostgreSQL local, `FORGE614_TEST_POSTGRES_BIN=$(pg_config --bindir)`):** las dos pruebas pasaban en `c3be0a4` y fallan desde T8 (`420275a`). Las 10 pruebas de PostgreSQL se omitían en todas las mediciones locales porque la variable no estaba puesta, aunque PostgreSQL está instalado en la Mac; D-T8-4 ajustó 21 pruebas pero no vio estas dos.
1. `setup refuses an incompatible PostgreSQL schema…` (`src/app/__tests__/postgres-sync.integration.test.ts:273`) y `configured CLI stays local offline…` (`tests/e2e/postgres-sync.test.ts:28` y `:38`): responden `["si", url, "no", "si"]`, pero con una base nueva el `init` ya no pregunta por el refuerzo (D-T8-3); el «no» cae en «¿Confirmar?» y cancela. Arreglo: `["si", url, "si"]`.
2. Con eso, la segunda falla con `SYNC_UPGRADE_REQUIRED` en la primera `syncWorkspace(config)` (`tests/e2e/postgres-sync.test.ts:36`): una base nueva (esquema 11) ya usa el formato 3 y la réplica recién creada está en el formato 1, así que subir el formato exige `sync --upgrade-format`. Es la protección que ya existía; antes la prueba la evitaba contestando «no» al refuerzo. Arreglo: `syncWorkspace(config, { upgradeFormat: true })` en esa primera sincronización.
3. **Consecuencia de D-T8-1, documentada:** con sincronización PostgreSQL, la primera `sync` de una base nueva contra una réplica en un formato anterior se detiene con `SYNC_UPGRADE_REQUIRED` hasta usar `sync --upgrade-format` (y todos los equipos de la réplica deben ser compatibles). Antes se podía evitar contestando «no» al refuerzo; ahora una base nueva ya no ofrece esa opción. El propietario no usa réplica (R2).

**Parche T10a ronda 2:** `2026-09-24-engram-1-7-0-t10-ci-fix.patch` (junto a este plan), SHA-256 `542ffcd25155114e136f3ed35828d63e73dc3b4a02f79be44f889a6683dd266e`, 75 líneas, 5 archivos: las dos pruebas, una frase en la viñeta «Bases nuevas con memoria inteligente» del CHANGELOG y una en el párrafo de sincronización del capítulo 05 es/en (sin entrada en `notion-map.json`). Laboratorio sobre `9855eac`: con PostgreSQL **682 pass / 1 skip / 0 fail**; sin PostgreSQL 673 / 10 / 0; typecheck 0; `git diff --check` limpio; aplicado con `git apply --check` sobre una copia nueva.
- Commit 1: `git add src/app/__tests__/postgres-sync.integration.test.ts tests/e2e/postgres-sync.test.ts && git commit -m "test(sync): new databases start at schema 11, so setup asks one question less and the first sync upgrades the format"`
...
1. **Hecho por el orquestador (2026-09-25 14:16:08Z = 08:16:08 hora local):** `forge614-engram update --json` → `{"updated":true,"previousVersion":"1.6.0","installedVersion":"1.7.0"}`; la base sigue en nivel 10 (1.7.0 se comporta como 1.6.0 hasta activar). En ese momento había 11 procesos `forge614-engram mcp` iniciados antes de la actualización (1.6.0).
2. El propietario cierra todas las sesiones de IA (Claude Code, Codex y cualquier otra) y abre solo la de T10b. Primer control de la sesión: `forge614-engram --version` = 1.7.0 y `ps -o pid,lstart,command -ax | grep 'forge614-engram mcp' | grep -v grep`: todo proceso debe haber empezado **después** de 2026-09-25 08:16:08 hora local; si queda alguno anterior, detenerse y listarlo (no matar procesos por su cuenta). Durante los pasos 3–5 la sesión no guarda nada en Engram (ni `memory_save` ni resúmenes), para que los conteos no cambien.
- Archivos actuales: `~/.forge614/engram/engram.db` (modo WAL, con `-wal` y `-shm`) y el respaldo previo `engram.db.v7-pre-ecosystem-20260923T182749941Z-8fb4a361.bak` (no se toca).
...
**Resultado T10b (2026-09-25):** sesión `54b2bce3` (Opus 5.5 high), a la primera. Primer control: 10 procesos `forge614-engram mcp` anteriores a la actualización (3 de la app de Codex, que mantiene sus servidores MCP vivos hasta salir con Cmd+Q); el agente se detuvo y los listó, y tras dos rondas de cierre solo quedó el suyo. A las 08:25 hora local (14:25:23Z) `intelligence-enable` → `enabled: true`, `schema: 11`, `migrated: true`, respaldo `engram.db.v10-pre-intelligence-20260925T142523750Z-81010a6e.bak` (`-rw-------`). Antes y después: 125 recuerdos (107 proyecto, 17 compartidos, 1 ecosistema), 216 versiones, 206 peticiones, 41 sesiones y las 5 huellas `.sha3sum` idénticas; `user_version` 10 → 11; `integrity_check` = `ok`; segunda activación `migrated: false`, `backup: null`. **Error del plan (inofensivo):** `.sha3sum` acepta un solo patrón por llamada, así que las cinco tablas van en cinco `.sha3sum <tabla>`; el agente lo repitió así. **Omisión del prompt:** al reescribir los pasos terminó en la segunda activación y dejó fuera la prueba de uso (paso 6). Verificado por el orquestador en solo lectura (`sqlite3 -readonly`): el respaldo está en el nivel 10 con los mismos conteos y huellas del «antes»; ninguna fila del respaldo falta o cambió en la base actual (`EXCEPT` = 0 en las 5 tablas; las filas nuevas son el recuerdo que guardó T10b tras el paso 5 y la sesión del orquestador); paso 6 con `memory_search`: «decision» encuentra recuerdos con «Decisión» y «migracion sesion» encuentra «migración»/«sesión». Agente: 1,14 M tokens ≈ $0,84 en 8,4 min. **T10b cerrada; Task 10 cerrada: Engram 1.7.0 publicado, instalado y activo en la base real.**
...
| T10a / T10b | **No** | Publicar e instalar no cambian cómo se integra un asistente, y la activación de la base del propietario tampoco: el punto de T8 ya cubre las bases nuevas y el nivel de una base existente. Consecuencia práctica: desde el 2026-09-25 la base de la Mac está en el esquema 11, así que los puntos que dicen «con esquema 11» (T2, T4, P-T3, P-T5, P-T6, P-T7b) ya se pueden verificar en esta Mac al revalidar la matriz con el reglamento 1.1.0. |
