...
- **Connections:** each relation in the contracts table (section 11 of the ecosystem contract) is one more trace of the same faint circuit, from the provider's circuit to the consumer's. An envelope in the provider's color travels along it; the middle shows what travels and, on hover, the contract source. The first one: Engram → Shell, "memoria · estado · MCP"; the envelope arrives just as Shell's terminal prints "engram listo".
- **Independence:** under each node's title, "● funciona solo" or "necesita: …", per the responsibilities table (section 2 of the contract).
