...
- **Conexiones:** cada relación de la tabla de contratos (sección 11 del contrato del ecosistema) es una pista más del mismo circuito tenue, del circuito de quien entrega al de quien recibe. Por ella viaja un sobre del color de quien entrega; a la mitad se lee qué viaja y, al pasar el mouse, la fuente del contrato. La primera: Engram → Shell, "memoria · estado · MCP"; el sobre llega justo cuando la terminal de Shell escribe "engram listo".
- **Independencia:** bajo el título de cada nodo, "● funciona solo" o "necesita: …", según la tabla de responsabilidades (sección 2 del contrato).
