...
- **Engram ("la memoria"):** una bóveda que trabaja sola, sin personas: un rack de servidores con luces que parpadean y, encima, un holograma de la red de neuronas (la idea original de Engram). Al pie del rack, del lado que mira a Shell, tiene dos conectores: uno de entrada y otro de salida. Cuando llega un recuerdo, la entrada destella, un pulso sube por el rack y el holograma brilla: el recuerdo quedó guardado. Cuando alguien pide recuerdos, el holograma brilla y un pulso baja hasta la salida. Su color es rosa suave (`#d98ca0`).
- **Conexiones:** la información viaja sola, sin nadie que la cargue: cables de fibra que salen de un conector y entran a otro, apoyados sobre las plataformas con clips metálicos y tensos en el aire entre ellas. Cada cable va en un solo sentido; si dos nodos se hablan en ambos sentidos, van dos cables juntos. Cada envío es un destello con estela que recorre el cable a velocidad pareja, y el conector que lo recibe se ilumina. Las relaciones salen de la tabla de contratos (sección 11 del contrato del ecosistema). Cada destello lleva el color de quien lo manda. Hasta ahora: Shell ↔ Engram (Shell guarda en azul, Engram devuelve recuerdos en rosa), Shell ↔ Engines (Shell pregunta en azul, Engines responde en ámbar), Atlas ↔ Shell, Atlas ↔ Engines, Atlas ↔ Engram y Atlas ↔ cada worker (Atlas en verde, workers en coral). Solo se dibuja como cable lo que realmente viaja entre dos nodos; una dependencia de instalación (por ejemplo, que Engram incluya a Engines) no es un cable y se mostrará en la capa de reglas y dependencias.
- **Un solo reloj:** todo el mapa sigue el ritmo de la terminal de Shell, en el orden real, y la terminal espera cada respuesta antes de escribir la línea que depende de ella. Shell escribe `forge614 prepare` y pregunta a la vez a Engram (recuerdos) y a Engines (motores de IA instalados); Engines los encuentra uno por uno y responde; Shell pide la vista previa del cambio y Engines la manda sin tocar nada; la persona confirma en la terminal (`¿aplicar cambios? (s/n) s`); Engines aplica solo eso y avisa (`cambios aplicados`); Shell pregunta si se quiere contextualizar el proyecto (`¿contextualizar el proyecto? (s/n) s`) y le dice a Atlas que empiece; Atlas pregunta a Engines qué motores pueden usar sus workers y a Engram si hay un avance guardado, avisa a Shell (`atlas: revisando repositorio…`), reparte las tareas a sus workers uno por uno, revisa cada informe, escribe lo aprendido en Engram (que lo guarda, le responde y lo copia a la nube) y da su reporte final a Shell (`contexto guardado en engram`); al final Shell guarda su resumen en Engram, que también lo copia a la nube. La terminal avanza hacia arriba cuando se llena. Nada se mueve por su cuenta. Una vuelta completa dura unos 42 segundos.
- **Atlas ("orquestador de contexto"):** hace la contextualización inicial del proyecto, opcional (acta 0004). Se dibuja como un cartógrafo en su mesa de dibujo: arriba flota una ciudad holográfica del repositorio (un edificio por carpeta, 24). Atlas no revisa el código él mismo: manda cada tarea a un worker y, cuando regresa el informe, una lupa holográfica salta de edificio en edificio de esa parte de la ciudad; cada carpeta revisada se vuelve sólida con un punto verde y las calles se iluminan, mientras la persona dibuja en su hoja el mapa de lo ya revisado y un panel cuenta las carpetas. Al terminar sube una tarjeta con lo aprendido, que viaja a Engram. Sus cables: con Shell (inicio y reportes), con Engines (motores), con Engram (avance previo y escritura, por dos conectores al frente del rack) y dos con cada worker. Su color es verde suave (`#86c2a4`).
- **Workers ("los obreros"):** tres nodos pequeños en abanico debajo de Atlas, sin circuito propio porque son parte de Workers. En cada uno, una persona trabaja en su computadora: mientras tiene una tarea, la pantalla se llena de código y la luz de su torre se pone coral; al terminar dice "informe enviado". Trabajan uno por uno y cada tarea empieza de cero (sin estado propio); solo le devuelven a Atlas el análisis crudo y nunca escriben en Engram (sección 7 del contrato). Cada uno tiene dos cables con Atlas: por uno recibe la tarea y por el otro regresa el resultado. Su color es coral (`#d9876f`).
- **Engines ("los motores"):** un banco de pruebas. Un proyector con aro ámbar lanza un haz de luz hacia un holograma de un motor funcionando (pistones que suben y bajan, polea que gira), recorrido siempre por un anillo escáner que va más rápido y brilla más mientras busca motores. Al lado flota un panel "motores de IA" con cuatro motores que pasan de "buscando…" a "listo", y el que recibe el cambio muestra "vista previa" y luego "aplicado"; al aplicar, el holograma se pone ámbar un momento. A la izquierda, de perfil, una persona de juguete sostiene con las dos manos una tableta con la misma lista y la toca mientras Engines trabaja. Engines no tiene pantalla propia ni cambia nada por su cuenta: todo pasa por Shell (sección 5 del contrato). Sus cables llegan a la base del proyector y, del lado de Shell, salen de una pequeña caja de red junto al escritorio. Su color es ámbar (`#d9a95b`).
- **Distribución:** los nodos quedan a 44 unidades entre sí, para que sus circuitos no se encimen: Shell y Engram lado a lado y, en la fila de abajo, Engines (centrado bajo ellos) y Atlas (a la derecha); los workers de Atlas van debajo de él.
- **La nube (PostgreSQL, opcional):** la copia opcional de Engram (sección 6 del contrato) se ve como un centro de datos remoto con tres racks, sin personas, flotando arriba y a la derecha de Engram, sin circuito debajo porque está fuera del ecosistema. Un cable de fibra sale del rack de Engram y entra al pie del rack central. Su tarjeta dice `NUBE · OPCIONAL`, en lila (`#a99bd6`).
...
- **Shell ("la terminal"):** una persona de juguete sentada en una silla de oficina frente a un escritorio de madera clara, con teclado, mouse y taza. Detrás, una pantalla holográfica flota sobre un pequeño proyector y muestra la sesión con comandos reales (`forge614 status`, `forge614 prepare`), la vista previa de Engines, la confirmación y el resultado; la persona teclea solo las líneas que le tocan (los comandos y su confirmación). Su color es azul suave (`#7fb2d9`).
